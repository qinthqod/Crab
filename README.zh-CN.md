...
[**下载 Crab v0.2.3 Apple Silicon 版 →**](https://github.com/qinthqod/Crab/releases/download/v0.2.3/Crab-0.2.3-macOS-arm64.dmg)

Crab 需要 Apple Silicon 与 macOS 14 或更高版本。安装前可下载
[SHA-256 校验文件](https://github.com/qinthqod/Crab/releases/download/v0.2.3/SHA256SUMS.txt)。
...
- **运行优化** — 用户主动点击后，体检整台 Mac 的磁盘、内存压力、Swap、连续运行时间、温度状态与持续高占用进程，再执行文件预览和应用关联维护；Finder、Dock 刷新保持可选，已挂载磁盘镜像仅在确认后推出。不会删除文件，也不会自动关闭第三方应用。
...
- **应用内更新** — Crab 每次启动时检查一次官方 GitHub Release 更新源；仅发现经过验证的新版本时在右上角提示，点击提示即可直接校验并安装。
...
- macOS 14 或更高版本
- 可下载的 v0.2.3 测试版适用于 Apple Silicon
- Intel Mac 可使用兼容的 Swift 6 工具链从源码构建
- 从源码构建需要 Swift 6 工具链

## 安装公开测试版

下载 [`Crab-0.2.3-macOS-arm64.dmg`](https://github.com/qinthqod/Crab/releases/download/v0.2.3/Crab-0.2.3-macOS-arm64.dmg)
和 [`SHA256SUMS.txt`](https://github.com/qinthqod/Crab/releases/download/v0.2.3/SHA256SUMS.txt)，
验证校验值并打开磁盘映像，然后把 `Crab.app` 拖到“Applications（应用程序）”快捷方式。
复制完成后推出 Crab 磁盘映像。

v0.1.1 增加了用户确认后的 Crab 应用内更新。由于 v0.1.0 尚未包含安装器，
从 v0.1.0 升级到 v0.1.1 是最后一次手动更新；之后的兼容版本可从 Crab 首页的更新提示或设置页面安装。
